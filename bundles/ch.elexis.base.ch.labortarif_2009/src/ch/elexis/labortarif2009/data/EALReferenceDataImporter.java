...
public class EALReferenceDataImporter extends AbstractReferenceDataImporter {
	...
	private void updateOrCreateFromImportedValues() {
		...
		if (openEntries.isEmpty()) {
			...
			Labor2009Tarif tarif = new Labor2009Tarif(importedValues.get(Labor2009Tarif.FLD_CHAPTER),
					importedValues.get(Labor2009Tarif.FLD_CODE), importedValues.get(Labor2009Tarif.FLD_TP),
					importedValues.get(Labor2009Tarif.FLD_NAME), importedValues.get(Labor2009Tarif.FLD_LIMITATIO),
					importedValues.get(Labor2009Tarif.FLD_FACHBEREICH), Fachspec.getFachspec(specs, row));
	...
	private void fillImportedValues2023(String[] line) {
		...
		importedValues.put(Labor2009Tarif.FLD_NAME, StringTool.limitLength(StringTool.getSafe(line, 3), 254));
		importedValues.put(Labor2009Tarif.FLD_LIMITATIO, StringTool.getSafe(line, 8));
		String isChem = StringTool.getSafe(line, 11);
		String isHaem = StringTool.getSafe(line, 12);
		String isImm = StringTool.getSafe(line, 13);
		String isGen = StringTool.getSafe(line, 14);
		String isMikro = StringTool.getSafe(line, 15);
		StringBuilder sb = new StringBuilder();
		if (isChem.equals("Ja")) {
			sb.append("C");
		}
		...
		importedValues.put(Labor2009Tarif.FLD_FACHBEREICH, sb.toString());
	}
	...
	private void fillImportedValues2012(String[] line) {
		...
		importedValues.put(Labor2009Tarif.FLD_NAME, StringTool.limitLength(StringTool.getSafe(line, 4), 254));
	...
	private void closeAllOlder() {
		...
		for (Labor2009Tarif labor2009Tarif : entries) {
			...
			if ((validFromString == null || validFromString.trim().length() == 0)) {
				...
				labor2009Tarif.set(Labor2009Tarif.FLD_GUELTIG_VON, defaultValidFrom.toString(TimeTool.DATE_COMPACT));
				labor2009Tarif.set(Labor2009Tarif.FLD_GUELTIG_BIS, validFrom.toString(TimeTool.DATE_COMPACT));
	...
	private int getFormatYear(String[] line) {
		...
		if (fach2012.equals("") && !fach2011.equals("")) {
			...
			return 2012;
		} else if (fach2011.equals("Resultat")) {
			return 2023;
		}
		return -1;
	}
