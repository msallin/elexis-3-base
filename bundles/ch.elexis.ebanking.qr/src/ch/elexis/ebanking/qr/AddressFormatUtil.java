...
public class AddressFormatUtil {
	...
	public static String getSingleLine(IPerson person) {
		...
		if (!StringTool.isNothing(person.getFamilyName())) {
			ret.append(person.getFamilyName() + StringTool.space);
		}
		if (!StringTool.isNothing(person.getFirstName())) {
			ret.append(person.getFirstName());
		}
		// date of birth
		TimeTool dateOfBirth = person.getDateOfBirth();
		if (dateOfBirth != null) {
		...
		if (!StringTool.isNothing(thisAddressMobile)) {
			ret.append(
					"," + StringTool.space + "Mobil:" + StringTool.space + thisAddressMobile);
		}
		String thisAddressFax = person.getFax();
		if (!StringTool.isNothing(thisAddressFax)) {
			ret.append("," + StringTool.space + "Fax:" + StringTool.space + thisAddressFax);
		}
		// email
	...
	public static String getFullnameWithSalutation(IContact contact) {
		...
		if (contact instanceof IPerson) {
			...
			sb.append(person.getFirstName()).append(StringTool.space).append(person.getFamilyName())
					.append(StringTool.lf);
		} else {
			sb.append(contact.getDescription1()).append(StringTool.space);
			if (StringUtils.isNotBlank(contact.getDescription2())) {
	...
	public static String getAddressPhoneFaxEmail(IContact contact, boolean multiline, boolean includePhone) {
		...
		if (includePhone) {
			...
			if (StringUtils.isNotBlank(contact.getMobile())) {
				thisAddress.append("Mobil:" + StringTool.space + contact.getMobile()
						+ System.lineSeparator());
			}
		}

		if (StringUtils.isNotBlank(contact.getFax())) {
			thisAddress.append("Fax:" + StringTool.space + contact.getFax()
					+ System.lineSeparator());
		}
